...
## 分页管理以及调取API时的错误信息
from api_exception import APIValueError, APIResourceNotFoundError, APIPermissionError, APIError
from models import Page, User, Comments, Blogs, next_id
...
async def get_blog(id):
    ...
    comments = await Comments.findAll('blog_id=?', [id], orderBy='created_at desc')
    for c in comments:
        c.html_content = markdown.markdown(c.content)
    blog.html_content = markdown.markdown(blog.content)
    return {
        '__template__': 'blog.html', # 通过配置的jinja2
        'blog': blog,
        'comments': comments
    }
...
async def api_comments(*, page='1'):
    page_index = get_page_index(page)
    num = await Comments.findNumber('count(id)')
    p = Page(num, page_index)
    if num == 0:
        return dict(page=p, comments=())
    comments = await Comments.findAll(orderBy='created_at desc', limit=(p.offset, p.limit))
...
async def api_create_comment(id, request, *, content):
    ...
    comment = Comments(blog_id=blog.id, user_id=user.id, user_name=user.name, user_image=user.image, content=content.strip())
    await comment.save()
    return comment
...
async def api_delete_users(id, request):
    ...
    await user.remove()
    # 给被删除的用户在评论中标记
    comments = await Comments.findAll('user_id=?', [id])
    if comments:
