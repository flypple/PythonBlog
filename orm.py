...
class ModelMetaclass(type):
    ...
    def __new__(cls, name, bases, attrs):
        ...
        logging.info('found nodel: %s (table: %s)' % (name, tableName))
        mappings = dict()
        fields = []
        primaryKey = None
        ...
        sql_select = 'select `%s`, %s from `%s`' % (primaryKey, ', '.join(escaped_fields), tableName)
        attrs['__select__'] = sql_select
        sql_insert = 'insert into `%s` (%s, `%s`) values (%s)' % (tableName, ', '.join(escaped_fields), primaryKey, create_args_string(len(escaped_fields) + 1))
        attrs['__insert__'] = sql_insert
        sql_update = 'update `%s` set %s where `%s`=?' % (tableName, ', '.join(map(lambda f: '`%s`=?' % (mappings.get(f).name or f), fields)), primaryKey)
        attrs['__update__'] = sql_update
        sql_delete = 'delete from `%s` where `%s`=?' % (tableName, primaryKey)
    ...
    async def findAll(cls, where=None, args=None, **kw):
        ...
        if limit is not None:
            ...
            if isinstance(limit, int):
                ...
                raise ValueError('Invalid limit value: %' % str(limit))
